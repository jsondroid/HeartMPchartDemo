                ...
                if (!dataSet.isStacked()) {
                    ...
                    for (int j = 0; j < buffer.buffer.length * mAnimator.getPhaseX(); j += 4) {
                        ...
                        if (dataSet.isDrawValuesEnabled()) {
                            Drawable icon1 = entry.getIcon();
                            float py = val >= 0 ?
                                    (buffer.buffer[j + 1] + posOffset) :
                                    (buffer.buffer[j + 3] + negOffset);//新加的
                            float ty=py+iconsOffset.y;
                            if(icon1!=null){
                                ty=py+iconsOffset.y+icon1.getIntrinsicHeight()/3;
                            }
                            drawValue(c, dataSet.getValueFormatter(), val, entry, i, x,ty,
